...
function EditItinerarySidebar({
    ...
    const actualItineraryData = {
        ...
        destinations: ensureArray(itineraryData?.destinations || localItineraryData.destinations)
    };
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            const res = await fetch("http://localhost:3000/api/update-itinerary", {
                method: "POST",
            ...
            if (res.ok && response.ok) {
                ...
                if (res.status === 401) {
                    ...
                    handleShowError('Failed to update itinerary on server. Please try again.', 'error');
                }
            }
        } catch (err) {
            console.error('Network error updating itinerary:', err);
            handleShowError('Error updating itinerary. Please check your connection and try again.', 'error');
        }
    };
    ...
    const canSave = actualItineraryData.title?.trim() && ensureArray(actualItineraryData.destinations).length > 0;
        ...
        <div className="create-itinerary-sidebar">
            ...
            <div className="create-actions">
                ...
                <button
                    ...
                    disabled={!canSave}
                >
                    Update Itinerary ({ensureArray(actualItineraryData.destinations).length} destinations)
                </button>
            </div>
