...
function Map({ onLocationSelect, isSelectingMode, selectedDestinations, onUpdateDestination, onCancelSelection }) {
    console.log('Map component props:', { isSelectingMode, selectedDestinationsCount: selectedDestinations?.length });

    return (
        <div className="map-container">
            <Wrapper
                apiKey={process.env.REACT_APP_GOOGLE_MAPS_API_KEY}
                render={(status) => render(status, { onLocationSelect, isSelectingMode, selectedDestinations, onUpdateDestination, onCancelSelection })}
                libraries={["places"]}
            >
            </Wrapper>
        </div>
...
const MapComponent = ({ onLocationSelect, isSelectingMode, selectedDestinations, onUpdateDestination, onCancelSelection }) => {
    const ref = React.useRef(null);
    const [map, setMap] = React.useState(null);
    const [markers, setMarkers] = React.useState([]);
    const [clickListener, setClickListener] = React.useState(null);

    console.log('MapComponent state:', { isSelectingMode, selectedDestinationsCount: selectedDestinations?.length });
        ...
        marker.addListener('dragend', (e) => {
            ...
            geocoder.geocode({ location: { lat: newLat, lng: newLng } }, (results, status) => {
                if (status === 'OK' && results[0]) {
                    ...
                    console.log('Geocoding failed, using coordinates - preserving name:', destination.name);
                    if (onUpdateDestination) {
                        onUpdateDestination(destination.id, {
                            lat: newLat,
                            lng: newLng,
        ...
        if (isSelectingMode && onLocationSelect) {
            ...
            geocoder.geocode({ location: { lat, lng } }, (results, status) => {
                ...
                if (status === 'OK' && results[0]) {
                    ...
                    const placeName = results[0].address_components.find(comp =>
                        comp.types.includes('establishment') ||
                        comp.types.includes('point_of_interest') ||
                        comp.types.includes('park')
                    )?.long_name || results[0].address_components[0]?.long_name || 'Selected Location';
    ...
    const handleOutsideClick = (e) => {
        if (ref.current && !ref.current.contains(e.target) && isSelectingMode && onCancelSelection) {
