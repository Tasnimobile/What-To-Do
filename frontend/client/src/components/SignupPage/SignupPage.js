...
const SignupPage = ({ onSignup, onSwitchToLogin, onBack, onGoogleLogin }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.ok && payload.ok) {
                ...
                const msg = payload.errors?.join(", ") || payload.raw || `Signup failed (status ${res.status})`;
    ...
    const handleChange = (e) => {
        ...
        if (e.target.name === 'password') setErrors(prev => ({ ...prev, password: '', confirmPassword: '' }));
        if (e.target.name === 'confirmPassword') setErrors(prev => ({ ...prev, confirmPassword: '' }));
    };
